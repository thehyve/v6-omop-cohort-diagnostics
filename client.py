...
def main():
    ...
    try:
        ...
        result_info = execute_cohort_diagnostics(algorithm_image, client, collaboration_id, names, omop_jsons,
                                                 organisations_to_include)

        if result_info and 'data' in result_info and len(result_info['data']) > 0:
            print("Extracting zip data from results...")
            result_data = result_info['data'][0]['result']
